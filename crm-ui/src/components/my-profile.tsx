...
// Reusable MyProfiles component extracted from page-level MyProfile implementation.
// Accepts an optional staffId; if not provided, defaults to current authenticated user.
// Does not perform routing (caller can handle navigation). 
...
export interface MyProfilesProps {
  cognitoSub?: string | null;          // Explicit staff id to show
  showBackButton?: boolean;         // Control showing back button
  onBack?: () => void;              // Back handler
  onDeleted?: () => void;           // Callback after deletion
  readOnly?: boolean;               // Force read-only mode
}

export function MyProfile({ cognitoSub, showBackButton = false, onBack, onDeleted, readOnly = false }: MyProfilesProps) {
  const { currentUser } = useCurrentUser();
  const { staff, loading, error, updateStaff, refetch } = useStaff({ staffId: null, staffSub: cognitoSub });
  const { deleteStaff, toggleStaffStatus } = useStaffManagement();

  const [isEditing, setIsEditing] = useState(false);
  const [deleteAlert, setDeleteAlert] = useState(false);
  const [toggleStatusAlert, setToggleStatusAlert] = useState<{ open: boolean; newStatus: boolean; }>({ open: false, newStatus: false });

  // If current user not yet loaded, wait (avoid flashing error)
  useEffect(() => {
    if (!cognitoSub && currentUser) {
      refetch();
    }
  }, [cognitoSub, currentUser, refetch]);

  if (loading) return <StaffProfileSkeleton />;
  ...
  const canEdit = !readOnly && (currentUser?.role === 'ADMIN' || parseInt(currentUser?.id || '0') === staff.id);
  ...
  const getRoleBadgeVariant = (roleType: string) => roleType === 'ADMIN' ? 'destructive' : 'outline';
          ...
          <div className="flex items-center space-x-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => setToggleStatusAlert({ open: true, newStatus: !staff.isActive })}>
                  {staff.isActive ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{ROLE_TYPE_DISPLAY[staff.roleType]}</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                {staff.isActive ? (
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{staff.isActive ? 'Active' : 'Inactive'}</p>
                </div>
              </div>
            </div>
          </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{StaffUtils.formatDate(staff.createdAt)}</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{StaffUtils.toDisplayView(staff).lastLoginDisplay}</p>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent>
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{staff.centerName || `Center ID: ${staff.centerId}`}</p>
              </div>
            </div>
          </CardContent>
        </Card>
